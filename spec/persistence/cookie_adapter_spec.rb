...
require_relative "../support/cookies_mock"

describe Funtestic::Persistence::CookieAdapter do

  let(:context) { double(:cookies => CookiesMock.new) }
  subject { Funtestic::Persistence::CookieAdapter.new(context) }
  ...
  it "handles invalid JSON" do
    context.cookies[:funtestic] = { :value => '{"foo":2,', :expires => Time.now }
    subject["my_key"].should be_nil
    subject["my_key"] = "my_value"
    subject["my_key"].should eq("my_value")
  end
