...
describe Funtestic::Helper do
  include Funtestic::Helper

  let(:experiment) {
    Funtestic::Experiment.find_or_create('link_color', 'blue', 'red')
  }
  ...
  describe "ab_test" do
    ...
    it "should increment the participation counter after assignment to a new user" do
      ...
      previous_red_count = Funtestic::Alternative.new('red', 'link_color').participant_count
      previous_blue_count = Funtestic::Alternative.new('blue', 'link_color').participant_count

      ab_test('link_color', 'blue', 'red')

      new_red_count = Funtestic::Alternative.new('red', 'link_color').participant_count
      new_blue_count = Funtestic::Alternative.new('blue', 'link_color').participant_count
    ...
    it 'should not increment the counter for an experiment that the user is not participating in' do
      ab_test('link_color', 'blue', 'red')
      e = Funtestic::Experiment.find_or_create('button_size', 'small', 'big')
      lambda {
    ...
    context "when store_override is set" do
      before { Funtestic.configuration.store_override = true }
    ...
    context "when on_trial_choose is set" do
      before { Funtestic.configuration.on_trial_choose = :some_method }
    ...
    it "should only let a user participate in one experiment at a time" do
      ...
      big = Funtestic::Alternative.new('big', 'button_size')
      big.participant_count.should eql(0)
      small = Funtestic::Alternative.new('small', 'button_size')
    ...
    it "should let a user participate in many experiment with allow_multiple_experiments option" do
      ...
      button_size_alt = Funtestic::Alternative.new(button_size, 'button_size')
  ...
  describe 'finished' do
    before(:each) do
      ...
      @experiment = Funtestic::Experiment.find_or_create(@experiment_name, *@alternatives)
      @alternative_name = ab_test(@experiment_name, *@alternatives)
      @previous_completion_count = Funtestic::Alternative.new(@alternative_name, @experiment_name).completed_count
    end

    it 'should increment the counter for the completed alternative' do
      finished(@experiment_name)
      new_completion_count = Funtestic::Alternative.new(@alternative_name, @experiment_name).completed_count
    ...
    it 'should not increment the counter for an experiment that the user is not participating in' do
      ...
      }.should_not change { Funtestic::Alternative.new('small', 'button_size').completed_count }
    end
    ...
    it 'should not increment the counter for an ended experiment' do
      ...
      lambda {
        finished('button_size')
      }.should_not change { Funtestic::Alternative.new(a, 'button_size').completed_count }
    end

    it "should not clear out the user's participation from their session" do
      ab_user.should eql(@experiment.key => @alternative_name)
      finished(@experiment_name)
      ab_user.should eql({@experiment.key => @alternative_name, @experiment.key + ':finished' => true})
    end
    ...
    it "should not reset the users session when experiment is not versioned" do
      ab_user.should eql(@experiment.key => @alternative_name)
      finished(@experiment_name)
      ab_user.should eql({@experiment.key => @alternative_name, @experiment.key + ':finished' => true})
    end

    it "should not reset the users session when experiment is versioned" do
      @experiment.increment_version
      @alternative_name = ab_test(@experiment_name, *@alternatives)
    ...
    #  before { Funtestic.configuration.on_trial_complete = :some_method }
    #  it "should call the method" do
    #    self.should_receive(:some_method)
    #    finished(@experiment_name)
    #  end
  ...
  context "finished with config" do
    it "passes reset option" do
      ...
      experiment = Funtestic::Experiment.find :my_experiment
  ...
  context "finished with metric name" do
    before { Funtestic.configuration.experiments = {} }
    before { Funtestic::Alternative.stub(:new).and_call_original }

    def should_finish_experiment(experiment_name, should_finish=true)
      alts = Funtestic.configuration.experiments[experiment_name][:alternatives]
      experiment = Funtestic::Experiment.find_or_create(experiment_name, *alts)
      alt_name = ab_user[experiment.key] = alts.first
      alt = double('alternative')
      alt.stub(:name).and_return(alt_name)
      Funtestic::Alternative.stub(:new).with(alt_name, experiment_name.to_s).and_return(alt)
      if should_finish
        alt.should_receive(:increment_completion)
        alt.should_receive(:increment_conversion)
      else
    ...
    it "passes reset option" do
      ...
      exp = Funtestic::Experiment.find :my_exp
  ...
  describe 'conversions' do
    it 'should return a conversion rate for an alternative' do
      ...
      previous_convertion_rate = Funtestic::Alternative.new(alternative_name, 'link_color').conversion_rate
      previous_convertion_rate.should eql(0.0)

      finished('link_color')

      new_convertion_rate = Funtestic::Alternative.new(alternative_name, 'link_color').conversion_rate
  ...
  describe 'when user is a robot' do
    ...
    describe 'finished' do
      it "should not increment the completed count" do
        ...
        previous_completion_count = Funtestic::Alternative.new(alternative_name, 'link_color').completed_count

        finished('link_color')

        new_completion_count = Funtestic::Alternative.new(alternative_name, 'link_color').completed_count
  ...
  describe 'when providing custom ignore logic' do
    context "using a proc to configure custom logic" do
      ...
      it "ignores the ab_test" do
        ...
        red_count = Funtestic::Alternative.new('red', 'link_color').participant_count
        blue_count = Funtestic::Alternative.new('blue', 'link_color').participant_count
  ...
  describe 'versioned experiments' do
    ...
    it "should reset the session of a user on an older version of the experiment" do
      alternative_name = ab_test('link_color', 'blue', 'red')
      ab_user.should eql({'link_color' => alternative_name})
      alternative = Funtestic::Alternative.new(alternative_name, 'link_color')
      ...
      new_alternative = Funtestic::Alternative.new(new_alternative_name, 'link_color')
  ...
  context 'when redis is not available' do
    ...
    before(:each) do
      Funtestic.stub(:redis).and_raise(Errno::ECONNREFUSED.new)
    end
    ...
    context 'and db_failover config option is turned on' do
      ...
      describe 'ab_test' do
        ...
        it 'should call db_failover_on_db_error proc with error as parameter' do
          ...
          Funtestic.configuration.db_failover_on_db_error.should_receive(:call)
          ab_test('link_color', 'blue', 'red')
        end
        ...
        context 'and preloaded config given' do
          before do
            Funtestic.configuration.experiments[:link_color] = {
              :alternatives => [ "blue", "red" ],
            }
          end
  ...
  context "with preloaded config" do
    ...
    it "pulls options from config file" do
      ...
      Funtestic::Experiment.new(:my_experiment).alternatives.map(&:name).should == [ "control_opt", "other_opt" ]
      Funtestic::Experiment.new(:my_experiment).goals.should == [ "goal1", "goal2" ]
    end
    ...
    it "fails gracefully if config is missing experiment" do
      Funtestic.configuration.experiments = { :other_experiment => { :foo => "Bar" } }
      lambda { ab_test :my_experiment }.should raise_error
    end

    it "fails gracefully if config is missing" do
      lambda { Funtestic.configuration.experiments = nil }.should raise_error
    end

    it "fails gracefully if config is missing alternatives" do
      Funtestic.configuration.experiments[:my_experiment] = { :foo => "Bar" }
      lambda { ab_test :my_experiment }.should raise_error
    end
  end

  it 'should handle multiple experiments correctly' do
    experiment2 = Funtestic::Experiment.find_or_create('link_color2', 'blue', 'red')
    alternative_name = ab_test('link_color', 'blue', 'red')
  ...
  context "with goals" do
    ...
    describe "finished" do
      ...
      it "should increment the counter for the specified-goal completed alternative" do
        @previous_completion_count_for_goal1 = Funtestic::Alternative.new(@alternative_name, @experiment_name).completed_count(@goal1)
        @previous_completion_count_for_goal2 = Funtestic::Alternative.new(@alternative_name, @experiment_name).completed_count(@goal2)
        finished({"link_color" => "purchase"})
        new_completion_count_for_goal1 = Funtestic::Alternative.new(@alternative_name, @experiment_name).completed_count(@goal1)
        new_completion_count_for_goal1.should eql(@previous_completion_count_for_goal1 + 1)
        new_completion_count_for_goal2 = Funtestic::Alternative.new(@alternative_name, @experiment_name).completed_count(@goal2)
